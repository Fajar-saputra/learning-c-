...
Penjual::Penjual(std::string nama, int umur, int lamaKerja, std::string alamat, std::string noTelp)
...
void Penjual::setNama(std::string nama) {
    if (!nama.empty()) {
        m_nama = nama;
    }
}
...
void Penjual::setAlamat(std::string alamat) {
    if (!alamat.empty()) {
        m_alamat = alamat;
    }
}
